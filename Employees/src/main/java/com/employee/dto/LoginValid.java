...
public class LoginValid {
	...
	public static boolean login(String id, String password) {
		...
		try {
			...
			JSONArray usersArray = (JSONArray) parser.parse(new FileReader(file));

			String hashedInputPassword = HashPassword.hashPassword(password);
			...
			for (Object obj : usersArray) {
				...
				if (jsonId.equals(id) && jsonPassword.equals(hashedInputPassword)) {

					LoginValid.id = jsonId;

					JSONArray roleArray = (JSONArray) user.get("role");

					roleArray.sort((o1, o2) -> o1.toString().compareTo(o2.toString()));
